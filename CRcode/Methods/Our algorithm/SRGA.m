...
%         final1 = greedyMapping(cur_assign,group1,group2);
...
%         final1 = greedyMapping(cur_assign, group1, group2);
%         final2 = greedyMapping(prev_assign, group1, group2);
        %if final2'*M*final2 > final1'*M*final1 % YtMY > XtMX 这个地方乘M就可以 不用乘problem.M 因为就差个常数项
        if prev_assign'*M*prev_assign > cur_assign'*M*cur_assign
            cur_assign = prev_assign;
        else % XtMX > YtMY
            prev_assign2 = prev_assign;
            prev_assign = cur_assign;
