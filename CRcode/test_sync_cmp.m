...
varyMinGrhCnt=40; % minimum graph count
varyMaxGrhCnt=40; % maximum graph count
grhTestCnt = 20;  %
target.random = 1;
target.fc = 0;
...
%     algNameSepSpace = '                    ';
    ...
    switch target.config.category
        ...
        case 'outlier'% same setting with 6th row in Table 1 in the PAMI paper 
            nInlier = 9;target.config.nOutlier = 1;target.config.deform = 0.05;
            target.config.density = 1;target.config.complete = 1;
            graphMinCnt = varyMinGrhCnt;graphMaxCnt = varyMaxGrhCnt;testCnt = grhTestCnt;
        ...
        case 'complete'% same setting with 8th row in Table 1 in the PAMI paper 
            nInlier = 10;target.config.nOutlier = 1;target.config.deform = 0.05;
...
algNameSepSpace = '                    ';
algSet.algNameSet = {'IMGM-D','IMGM-R','CR-D','CR-R','CR-kmeans'};

filename = ['CR-IMGM-','.csv'];
fidPerf = fopen(filename,'w');
T = clock;
fprintf('time: %d : %d : %f\n', T(4), T(5), T(6));
...
for iGraphCnt = 1:length(graphCntList) % for each number of graphs
    ...
    for iTest = 1:grhTestCnt   % for each independent test
        affinity = generateRandomAffinity(nInlier,iTest,4);
        ...
        for i = 1:IMGMcount
            ...
            %%%%%%%%%%%%%%%%%%%%% SCM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if i == 1
                SCMPrevMatching = baseMat; % DPP                
                scrSCM = cal_pair_graph_score(baseMat,affinity.GT,nodeCnt,baseGraphCnt);
                conSCM = cal_pair_graph_consistency(baseMat,nodeCnt,baseGraphCnt,0);
                idx = ones(1,30);
            end
            
            SCMMatTmp = rawMat(1:end-nodeCnt*(IMGMcount - i),1:end-nodeCnt*(IMGMcount - i));
            SCMMatTmp(1:end-nodeCnt,1:end-nodeCnt)=SCMPrevMatching;
            
            scrDenomMatInCntTmp = cal_pair_graph_inlier_score(SCMMatTmp,affinity.GT(1:nodeCnt*(baseGraphCnt+i),1:nodeCnt*(baseGraphCnt+i)),nodeCnt,baseGraphCnt+i,nodeCnt);
            conDenomMatInCntTmp = cal_pair_graph_consistency(SCMMatTmp,nodeCnt,baseGraphCnt+i,0);
            ...
            param.method = 1;
            tStart = tic;
            [SCMIncreMatching,idx] = SCM(simAP, SCMMatTmp, param, idx, scrSCM, conSCM);
            tEnd = toc(tStart);
            SCMPrevMatching = SCMIncreMatching;
            
            accSCM = cal_pair_graph_accuracy(SCMIncreMatching,affinity.GT,target.config.nOutlier,nodeCnt,baseGraphCnt+i);
            scrSCM = cal_pair_graph_score(SCMIncreMatching,affinity.GT,nodeCnt,baseGraphCnt+i);
            conSCM = cal_pair_graph_consistency(SCMIncreMatching,nodeCnt,baseGraphCnt+i,0);
            
            accResult(5,i,iTest)=mean(accSCM(:));
            scrResult(5,i,iTest)=mean(scrSCM(:));
            conResult(5,i,iTest)=mean(conSCM(:));
            timResult(5,i,iTest)=tEnd;
